...
static bool sg_color_invert = 0;

#if ED_HELPER_BUFFER_DRAW_FUNC_ENABLE
static ed_helper_buffer_t *sg_helper_buffer = NULL;
#endif
...
#if ED_HELPER_BUFFER_DRAW_FUNC_ENABLE
		if (sg_helper_buffer != NULL)
		{
			easy_draw_buffer_set_pixel(x, y, color);
			continue;
		}
#endif
		if (x >= 0 && x < sg_screen_width && y >= 0 && y < sg_screen_height)
		{
			color = easy_helper_color_invert(color);
...
static void easy_draw_brush(int16_t x, int16_t y, uint32_t color)
{
	...
	for (int i = -brush_half_size_s; i <= brush_half_size_e; i++)
	{
		for (int j = -brush_half_size_s; j <= brush_half_size_e; j++)
		{
			...
			if (sg_helper_buffer != NULL)
			{
				easy_draw_buffer_set_pixel(xi, yj, color);
				continue;
			}
#endif
...
{
	// TODO:
	if (sg_color_invert == 0)
		return color;

#if (ED_COLOR_DEPTH == 1)
	color = !color;
#endif
	// color |= (255 - (color & 0xff));
	// color |= (255 - ((color >> 8) & 0xff)) << 8;
	// color |= (255 - ((color >> 16) & 0xff)) << 16;
	// color |= (255 - ((color >> 24) & 0xff)) << 24;
	return color;
}
...
{
	if (ED_IS_DRIVER_NULL(sg_ed_driver, ed_pixel_read))
		return ED_ERROR;

	if (x >= 0 && x < sg_screen_width && y >= 0 && y < sg_screen_height)
	{
		return sg_ed_driver->ed_pixel_read(x, y, color);
	}
...
void easy_draw_fillSector(int16_t xc, int16_t yc, int16_t r, int16_t start_angle, int16_t end_angle, uint32_t color)
{
	...
	int16_t sx, sy, ex, ey;
	float radians, start_radians, end_radians;
	ed_point_t triangle_points[3] = {0};
...
void easy_draw_circle(int16_t x0, int16_t y0, uint16_t r, uint32_t color)
{
	...
	easy_draw_point(x0, y0 + r, color);
	easy_draw_point(x0, y0 - r, color);
	easy_draw_point(x0 + r, y0, color);
	easy_draw_point(x0 - r, y0, color);
	...
	while (x < y)
	{
		...
		easy_draw_point(x0 + x, y0 + y, color);
		easy_draw_point(x0 - x, y0 + y, color);
		easy_draw_point(x0 + x, y0 - y, color);
		easy_draw_point(x0 - x, y0 - y, color);
		easy_draw_point(x0 + y, y0 + x, color);
		easy_draw_point(x0 - y, y0 + x, color);
		easy_draw_point(x0 + y, y0 - x, color);
		easy_draw_point(x0 - y, y0 - x, color);
	}
...
void easy_helper_fillCircle(int16_t x0, int16_t y0, uint16_t r, uint8_t sides, int16_t delta, uint32_t color)
{
	int16_t f = 1 - r;
	int16_t ddF_x = 1;
	int16_t ddF_y = -2 * r;
...
void easy_helper_circle(int16_t x0, int16_t y0, uint16_t r, uint8_t corners, uint32_t color)
{
	int16_t f = 1 - r;
	int16_t ddF_x = 1;
	int16_t ddF_y = -2 * r;
...
void easy_draw_ellipse2(int16_t x0, int16_t y0, int16_t x1, int16_t y1, uint32_t color)
{
	...
	do
	{
		easy_draw_point(x1, y0, color);
		easy_draw_point(x0, y0, color);
		easy_draw_point(x0, y1, color);
		easy_draw_point(x1, y1, color);
		e2 = 2 * err;
		if (e2 >= dx)
		{
			x0++;
	...
	while (y0 - y1 < b)
	{
		easy_draw_point(x0 - 1, y0, color);
		easy_draw_point(x1 + 1, y0++, color);
		easy_draw_point(x0 - 1, y1, color);
		easy_draw_point(x1 + 1, y1--, color);
	}
}
...
void easy_draw_ellipse(int16_t xCenter, int16_t yCenter, int16_t Rx, int Ry, uint32_t color)
{
	...
	int32_t px = 0;
	int32_t py = twoRx2 * y;
	easy_draw_point(xCenter + x, yCenter + y, color);
	easy_draw_point(xCenter - x, yCenter + y, color);
	easy_draw_point(xCenter + x, yCenter - y, color);
	easy_draw_point(xCenter - x, yCenter - y, color);
	// Region?1
...
ed_polygonRect_helper_t easy_helper_getPolygonRect(uint16_t num_points, ed_point_t tar_polygon[])
{
	...
	for (int i = 0; i < num_points; i++)
	{
		...
		if (tar_polygon[i].x < boxRect.minX)
			boxRect.minX = tar_polygon[i].x;
		if (tar_polygon[i].y < boxRect.minY)
			boxRect.minY = tar_polygon[i].y;
		if (tar_polygon[i].x > boxRect.maxX)
			boxRect.maxX = tar_polygon[i].x;
		if (tar_polygon[i].y > boxRect.maxY)
			boxRect.maxY = tar_polygon[i].y;
	}
	return boxRect;
}
...
// https://www.codetd.com/article/11761841
	...
	for (testPoint.x = polyRect.minX; testPoint.x < polyRect.maxX; testPoint.x++)
		for (testPoint.y = polyRect.minY; testPoint.y < polyRect.maxY; testPoint.y++)
		{
			...
			if (easy_helper_inPolygon(num_points, points, testPoint))
				easy_draw_point(testPoint.x, testPoint.y, color);
		}
}
...
void easy_draw_clear(int16_t x, int16_t y, uint16_t width, uint16_t height, uint32_t color)
{
	...
	easy_draw_fillRect(x, y, width, height, color);
}
...
void easy_draw_full_clear(uint32_t color)
{
	...
	color = easy_helper_color_invert(color);
	if (ED_IS_DRIVER_NULL(sg_ed_driver, ed_screen_buffer_clear))
	{
		easy_draw_clear(0, 0, sg_screen_width, sg_screen_height, color);
	}
	else
	{
		sg_ed_driver->ed_screen_buffer_clear(color);
	}
}

#if ED_HELPER_BUFFER_DRAW_FUNC_ENABLE

ed_helper_buffer_t *easy_draw_create_buffer(uint16_t w, uint16_t h, uint8_t *data)
{
	ed_helper_buffer_t *ed_helper_buffer = (ed_helper_buffer_t *)malloc(sizeof(ed_helper_buffer_t));
	if (ED_IS_NULL(ed_helper_buffer))
		return NULL;

	ed_helper_buffer->width = w;
	ed_helper_buffer->height = h;
	ed_helper_buffer->data = data;

	return ed_helper_buffer;
}

void easy_draw_set_buffer_active(ed_helper_buffer_t *buffer)
{
	sg_helper_buffer = buffer;
}

void easy_draw_set_buffer_inactive()
{
	sg_helper_buffer = NULL;
}

static void easy_helper_buffer_2bit_cover(int16_t x, int16_t y, uint16_t w, uint16_t h, uint8_t *buffer)
{
	int16_t sx = 0, sy = 0;
	for (int xi = 0; xi < w; xi++)
	{
		sx = xi + x;
		if (sx < 0 || sx > sg_screen_width)
			continue;
		for (int yi = 0; yi < h; yi++)
		{
			sy = yi + y;
			if (sy < 0 || sy > sg_screen_height)
			{
				continue;
			}

			uint32_t color = easy_helper_color_invert((buffer[(yi / 8 * w) + xi] >> (yi % 8)) & 0x01);
			sg_ed_driver->ed_pixel_draw(sx, sy, color ? 1 : 0);
		}
	}
}
...
void easy_draw_buffer_cover(int16_t x, int16_t y, ed_helper_buffer_t *buffer)
{
		...
		easy_helper_buffer_2bit_cover(x, y, buffer->width, buffer->height, buffer->data);
#endif
...
{
	if (ED_IS_NULL(sg_helper_buffer))
		return;
	int16_t w = sg_helper_buffer->width;
	int16_t h = sg_helper_buffer->height;
	...
	if (color)
	{
		ED_SET_BIT(sg_helper_buffer->data[y / 8 * w + x], (y % 8));
	}
	else
	{
		ED_CLR_BIT(sg_helper_buffer->data[y / 8 * w + x], (y % 8));
	}
#endif
...
{
	if (x > sg_helper_buffer->width || x < 0 || y > sg_helper_buffer->height || y < 0)
	{
		if (error != NULL)
			*error = -1;
		return 0;
	...
	if (error != NULL)
		*error = -1;
	return (sg_helper_buffer->data[(y / 8 * sg_helper_buffer->width) + x] >> (y % 8)) & 0x01;
...
#endif
// Horizontal scanning
void easy_draw_bit_pic(int16_t x, int16_t y, const uint8_t *picdata, uint8_t w, uint8_t h, uint8_t isOverlay)
{
	uint8_t bit_color = 0;
...
void easy_draw_2bit_pic(int16_t x, int16_t y, const uint8_t *picdata, uint8_t w, uint8_t h, uint8_t isOverlay)
{
	...
	for (yi = 0; yi < h; yi++)
	{
		for (xi = 0; xi < w; xi++)
		{
			...
			easy_draw_pixel(x + xi, y + yi, bit_color / 4);
		}
	}
}

void easy_draw_pic(int16_t x, int16_t y, const uint8_t *picdata, uint8_t w, uint8_t h, uint8_t isOverlay)
{
#if (ED_COLOR_DEPTH == 1)
	easy_draw_bit_pic(x, y, picdata, w, h, isOverlay);
#else if (ED_COLOR_DEPTH == 2)
	easy_draw_2bit_pic(x, y, picdata, w, h, isOverlay);
#endif
}

#if ED_TEXT_DRAW_FUNC_ENABLE
ed_text_format_config_t sg_ed_text_config = {1, 0, 1};
...
void easy_draw_set_text_size(uint8_t text_size)
{
	...
	sg_ed_text_config.size = text_size;
}

void easy_draw_get_text_size(uint8_t *re_text_size)
{
	*re_text_size = sg_ed_text_config.size;
}

void easy_draw_set_text_format(uint8_t text_size, uint32_t bk_color, uint8_t bk_color_isValid)
{
	sg_ed_text_config.size = text_size;
	sg_ed_text_config.bk_color = bk_color;
	sg_ed_text_config.bk_color_isValid = bk_color_isValid;
}
...
void easy_draw_text_char(int16_t x, int16_t y, uint8_t c, uint32_t color)
{
	int16_t i, j;
	uint8_t a, b;
	uint8_t size = sg_ed_text_config.size;
	uint32_t text_color = color;
	uint32_t bk_color = sg_ed_text_config.bk_color;
	uint32_t bk_color_isValid = sg_ed_text_config.bk_color_isValid;
...
{
	if (ED_IS_DRIVER_NULL(sg_ed_driver, ed_sync_update))
		return;

	sg_ed_driver->ed_sync_update();
}

void easy_draw_info_show(void)
{
	uint8_t size = 1;
	char *showstr = "WELCOME";
	easy_draw_get_text_size(&size);
	uint16_t cenx = sg_screen_width / 2;
	cenx = cenx - (strlen(showstr) * 6 * size) / 2;

	easy_draw_clear(0, 0, sg_screen_width, sg_screen_height, 0);
	easy_draw_printf(cenx, 0, ED_TEXT_DEFAULT_COLOR_SHOW, showstr);

	cenx = sg_screen_width / 2 - (strlen("easy_draw " EASY_DRAW_VERSION_STRING) * 6 * size) / 2;

	easy_draw_printf(cenx, sg_screen_height / 3, ED_TEXT_DEFAULT_COLOR_SHOW, "easy_draw " EASY_DRAW_VERSION_STRING);

	cenx = sg_screen_width / 2 - (strlen("author: " EASY_DRAW_AUTHOR_STRING) * 6 * size) / 2;
	easy_draw_printf(cenx, sg_screen_height / 3 * 2, ED_TEXT_DEFAULT_COLOR_SHOW, "author: " EASY_DRAW_AUTHOR_STRING);
